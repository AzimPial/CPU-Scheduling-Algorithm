/**
 * @fileoverview SchedViz — Dark/light theme toggle with no-FOUC.
 * @module ui/theme
 */

import { getTheme, setTheme as saveTheme } from '../core/storage.js';

/**
 * Initialize the theme system.
 * Call the returned function to toggle.
 * @param {HTMLElement} toggleBtn - The theme toggle button element
 * @returns {{toggle: () => void, get current: () => string}}
 */
export function initTheme(toggleBtn) {
  let current = getTheme();
  applyTheme(current);

  function applyTheme(theme) {
    document.documentElement.setAttribute('data-theme', theme);
    current = theme;
    updateIcon();
  }

  function updateIcon() {
    if (!toggleBtn) return;
    if (current === 'dark') {
      toggleBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>';
      toggleBtn.setAttribute('aria-label', 'Switch to light theme');
    } else {
      toggleBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>';
      toggleBtn.setAttribute('aria-label', 'Switch to dark theme');
    }
  }

  function toggle() {
    current = current === 'dark' ? 'light' : 'dark';
    applyTheme(current);
    saveTheme(current);
  }

  if (toggleBtn) {
    toggleBtn.addEventListener('click', toggle);
  }

  return {
    toggle,
    get current() { return current; }
  };
}
